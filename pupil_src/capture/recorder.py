...
class Recorder(Plugin):
    """Capture Recorder"""
    def __init__(self,g_pool,session_name = get_auto_name(),rec_dir=None, user_info={'name':'rename_me','additional_field':'change_me'},info_menu_conf={},show_info_menu=False, record_eye = False, audio_src = 'No Audio',raw_jpeg=True):
    ...
    def update(self,frame,events):
        if self.running:
            for key,data in events.iteritems():
                if key not in ('dt'):
                    # Mohammad - Not important - I just want to check that Crowd_eye plugin receives same number of frames
                    self.total_frames += 1
                    # end
                    try:
    ...
    def stop(self):
        ...
        # Mohammad - Temp code not important
        print 'total_frames_Recorder', self.total_frames
        # end
